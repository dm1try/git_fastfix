...
function FixupToTheCommit()
  ...
  nvim_call_function('system', { "git commit --fixup " .. commit_hash })
  nvim_call_function('system', { "GIT_EDITOR=true git rebase -i --autosquash --autostash " .. commit_hash .. "^" })
...
function ShowGitLog()
  ...
  local git_log_output = nvim_call_function('system', {'git log -n 10 --oneline'})
...
function OpenGitFastFixWindow()
  ...
  nvim_call_function('termopen', {'git add --patch ' .. source_code_filename})
